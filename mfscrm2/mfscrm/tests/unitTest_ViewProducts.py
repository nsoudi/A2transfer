...
class ll_ATS(unittest.TestCase):
    ...
    def test_ll(self):
        ...
        elem = driver.find_element(By.XPATH, '//*[@id="myNavbar"]/ul[1]/li[4]/a')
        elem.send_keys(Keys.RETURN)
        time.sleep(3)  # pause to allow screen to change
        try:
            # verify Edit button exists on new screen after clicking "Edit" button
            elem = driver.find_element(By.XPATH,
                                       '//*[@id="app-layout"]/div/div/div/div[3]/table/tbody/tr/td[7]/a')
            elem.send_keys(Keys.RETURN)
            print("Test passed - Product list displayed")
            assert True

        except NoSuchElementException:
            self.fail("Product List does not appear when Product is clicked - test failed")
